...
<html>
    ...
    <body>
        ...
        <?php
            ...
            //get Company
            $query = 'SELECT C.name FROM Company C WHERE C.id=' . $_GET['id'];
            ...
            if(!$substmnt->bind_result($company)) {
            ...
            //get movies
            $query = 'SELECT C.name, M.title FROM Company C INNER JOIN'
            . ' Movie M ON C.id = M.company_id WHERE C.id=' . $_GET['id'];
            ...
            if(!$substmnt->bind_result($company, $title)) {
                echo 'Bind failed: ' .  $mysqli->connect_errno . ' ' . $mysqli->connect_error;
            }
            ...
            while($stmnt->fetch()) {
                echo '<h1>Details for ' . $company . '</h1>';
